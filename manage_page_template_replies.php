<?php
// manage_page_template_replies.php
// هذا الملف مسؤول عن إدارة الردود العشوائية لقالب صفحة محدد.
...
if (!isset($_GET['template_id']) || !is_numeric($_GET['template_id'])) {
    // إذا لم يتم توفير معرف القالب أو كان غير صالح، أعد التوجيه إلى صفحة الصفحات
    header("location: pages.php"); // العودة لصفحة الصفحات لأن هذا القالب يخص صفحة
    exit();
}

$template_id = $_GET['template_id']; // معرف القالب المخصص للصفحة في قاعدة البيانات المحلية

// 3. التحقق من أن القالب يتبع الصفحة الصحيحة وأن الصفحة تتبع المستخدم الحالي
$stmt = $pdo->prepare("SELECT rt.*, fp.page_name 
                       FROM reply_templates rt
                       JOIN facebook_pages fp ON rt.page_id = fp.id
                       WHERE rt.id = :template_id AND fp.user_id = :user_id");
$stmt->execute([':template_id' => $template_id, ':user_id' => $current_user_id]);
$page_template = $stmt->fetch();

// إذا لم يتم العثور على القالب أو كانت لا تتبع المستخدم الحالي
if (!$page_template) {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] == 'add_content') {
        ...
        if (!empty($content)) {
            $stmt = $pdo->prepare("INSERT INTO reply_templates_contents (template_id, content) VALUES (:template_id, :content)");
            $stmt->execute([':template_id' => $template_id, ':content' => $content]);
            
            // إعادة التوجيه لمنع إعادة إرسال النموذج عند تحديث الصفحة
            header("location: manage_page_template_replies.php?template_id=" . $template_id);
            exit();
        } else {
    ...
    if (isset($_POST['action']) && $_POST['action'] == 'edit_content') {
        ...
        if (!empty($content)) {
            ...
            $stmt = $pdo->prepare("UPDATE reply_templates_contents SET content = :content WHERE id = :content_id AND template_id = :template_id");
            $stmt->execute([
                ':content' => $content,
    ...
    if (isset($_POST['action']) && $_POST['action'] == 'delete_content') {
        ...
        $stmt = $pdo->prepare("DELETE FROM reply_templates_contents WHERE id = :content_id AND template_id = :template_id");
...
$stmt = $pdo->prepare("SELECT * FROM reply_templates_contents WHERE template_id = :template_id ORDER BY id DESC");
...
<head>
    ...
    <title>إدارة الردود لـ "<?php echo htmlspecialchars($page_template['template_name']); ?>" (صفحة: <?php echo htmlspecialchars($page_template['page_name']); ?>)</title>
    <link rel="stylesheet" href="style.css">
</head>
<body>
    <?php include 'navbar.php'; // تضمين شريط التنقل العلوي ?>
    <div class="wrapper">
        <h2>إدارة الردود لمجموعة القوالب: "<?php echo htmlspecialchars($page_template['template_name']); ?>"</h2>
        <p>مخصصة للصفحة: "<?php echo htmlspecialchars($page_template['page_name']); ?>"</p>
        <a href="edit_templates.php?page_id=<?php echo htmlspecialchars($page_template['page_id']); ?>" class="btn btn-secondary">العودة إلى قوالب الصفحة</a>
